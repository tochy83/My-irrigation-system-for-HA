...
if max_num == 0:
    print("Aucun fichier voie_X.yaml trouvé.", file=sys.stderr)
    sys.exit(1)

if max_num == 1:
    print("Vous ne pouvez pas supprimer les fichiers de la voie 1.", file=sys.stderr)
    sys.exit(1)

# --- Liste des fichiers à supprimer ---
fichiers_a_supprimer = [
    os.path.join(DOSSIER, f"voie_{max_num}.yaml"),
    os.path.join(DOSSIER, f"automatisation_voie_{max_num}.yaml"),
]
...
for fichier in fichiers_a_supprimer:
    if os.path.exists(fichier):
        try:
            os.remove(fichier)
            print(f"Fichiers voies supprimés : {os.path.basename(fichier)}", file=sys.stderr)
